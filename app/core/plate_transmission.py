...
class Plate:
    def __init__(self, total_time=500, lx=120e-3, lz=120e-3, thickness=1.5e-3, nx=120, nz=120, k=205, rho=2700,
        ...
        #         self.positions.append((i*self.dx,j*self.dz))
        # self.positions_z = np.arange(0, self.nz) * self.dz  # Position z vector
        ...
        # self.p_in_location_z = int(round((lz / 2) / self.dx))  # Location of power input (halfway for the depth)
        self.powers = np.zeros([nx, nz])
        self.powers[self.p_in_location] = power_in  # Power applied to one element
        # self.powers_z = np.zeros(nz)
        # self.powers_z[self.p_in_location_z] = power_in  # Power applied to one element

        # Initial conditions
        self.ambient_temp = 273.0 + ambient_temp  # Ambient temperature [K]
        self.temps = np.full([nx, nz], self.ambient_temp)  # Initial temperature of all elements in x
        # print(self.temps)
        # self.temps_z = np.full(nz, self.ambient_temp)  # Initial temperature of all elements in x
        ...
        # self.temp_location_z = int(round((lz / 2) / self.dz))  # Location of localized heat
        # self.thermistance_location_z = int(round((3 * lz / 4) / self.dz))  # Location of temperature measurement
        ...
        # self.new_temps_z = np.zeros_like(self.temps_z)
            ...
            for j in range(self.nz):
                ...
                if i == 0:  # First column
                    if j == 0: # First row
                        ...
                        self.new_temps[i, j] += self.dt / (self.rho * self.cp) * self.k * (
                            self.temps[i + 1, j] - self.temps[i, j]) / self.dx**2 # right
                        self.new_temps[i, j] += self.dt / (self.rho * self.cp) * self.k * (
                            self.temps[i, j + 1] - self.temps[i, j]) / self.dz**2 # up
                        
                        self.new_temps[i, j] += self.dt / (self.rho * self.cp) * self.h_convection * (
                ...
                self.new_temps[i, j] += self.dt / (self.rho * self.cp) * self.h_convection * (self.ambient_temp - self.temps[i, j]) * 2 * self.area_top / self.volume
